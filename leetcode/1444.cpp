...
public:
    int ways(vector<string>& pizza, int k) {
        ...
        vector apples(rows+1, vector<int> (cols+1, 0));
        vector dp(k, vector (rows, vector<int> (cols, 0)));
...
// and horizontal cuts such that the piece of pizza you have to give a person must contain at least one apple. The base case is when c=k-1.
